...
export async function getRecipeRecommendations(ingredients, dietaryPreferences = []) {
  try {
    const prompt = `Generate recipe recommendations using these ingredients: ${ingredients.join(", ")}
    ${dietaryPreferences.length ? `Consider these dietary preferences: ${dietaryPreferences.join(", ")}` : ""}
    
    Return an array of recipe objects with this exact structure:
    {
      "name": "string",
      "description": "string",
      "ingredients": ["string"],
    ...
    const response = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are a professional chef specializing in creative recipes using available ingredients."
        },
        { role: "user", content: prompt }
      ],
      model: "llama-3.3-70b-versatile",
    ...
    const content = response.choices[0]?.message?.content
    if (!content) {
      throw new Error("No response from recipe recommendations")
    }
    ...
    try {
      ...
      console.error("Failed to parse recipe recommendations:", error)
      throw new Error("Invalid recipe format received")
    }
  } catch (error) {
    console.error("Error with Groq API:", error)
