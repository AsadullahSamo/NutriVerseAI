...
export async function getEquipmentRecommendations(currentEquipment, cookingPreferences, budget) {
...
async function fallbackToBackendRecommendations(currentEquipment, cookingPreferences, budget) {
    try {
...
export async function generateMaintenanceSchedule(equipment, startDate, endDate) {
    try {
...
function generateDateBasedOnCondition(equipment, startDate, endDate) {
    ...
    const start = new Date(startDate);
    const end = new Date(endDate);
    const finalDate = new Date(Math.max(Math.min(scheduledDate.getTime(), end.getTime()), start.getTime()));
...
async function fallbackToBackendMaintenanceSchedule(equipment, startDate, endDate) {
...
async function fallbackToBackendRecipeMatches(equipment, userPreferences) {
    try {
        ...
        if (!response.ok)
            throw new Error('Failed to get recipe matches');
        return response.json();
    }
    catch (error) {
