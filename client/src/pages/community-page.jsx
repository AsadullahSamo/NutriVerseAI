...
export default function CommunityPage() {
    ...
    const filteredPosts = selectedType
        ? posts === null || posts === void 0 ? void 0 : posts.filter(post => post.type === selectedType &&
            !hiddenPosts.includes(post.id))
        : posts === null || posts === void 0 ? void 0 : posts.filter(post => !hiddenPosts.includes(post.id));
    if (isLoading) {
    ...
    return (<div className="min-h-screen bg-background">
      <div className="container mx-auto p-6">
        <header className="mb-8">
          ...
          {!currentGoal && (filteredPosts === null || filteredPosts === void 0 ? void 0 : filteredPosts.some(post => post.type === "RECIPE_SHARE")) && (<div className="mb-6 mt-6 p-4 rounded-lg border bg-muted/50">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="mt-2">
                    <Button variant="outline" size="sm" onClick={() => window.location.href = '/nutrition'}>
                      Set Nutrition Goals
                    </Button>
                  </div>
                </div>
          ...
          <div className="flex gap-2 mt-6">
            ...
            <Button variant={selectedType === "RECIPE_SHARE" ? "default" : "outline"} onClick={() => setSelectedType("RECIPE_SHARE")}>
              Recipes
            </Button>
            <Button variant={selectedType === "FOOD_RESCUE" ? "default" : "outline"} onClick={() => setSelectedType("FOOD_RESCUE")}>
              Food Rescue
            </Button>
            <Button variant={selectedType === "COOKING_TIP" ? "default" : "outline"} onClick={() => setSelectedType("COOKING_TIP")}>
              Tips
            </Button>
          </div>
        </header>
        ...
        <div className="grid gap-6">
          {filteredPosts === null || filteredPosts === void 0 ? void 0 : filteredPosts.map((post) => (<Card key={post.id}>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Users className="h-4 w-4 text-muted-foreground"/>
                    <span className="text-sm text-muted-foreground">
                      Posted by {post.userId === (user === null || user === void 0 ? void 0 : user.id) ? "you" : post.username}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    {post.userId === (user === null || user === void 0 ? void 0 : user.id) ? (<>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => deleteMutation.mutate(post.id)} className="bg-destructive text-destructive-foreground">
                                Delete Post
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                ...
                {post.type === "RECIPE_SHARE" && (<>
                    ...
                    {post.recipe ? (<RecipeCard recipe={Object.assign(Object.assign({}, post.recipe), { postId: post.id })} hideEditDelete={true}/>) : (<div className="p-4 border rounded-md bg-muted">
                        <p className="text-sm text-muted-foreground">This recipe has been deleted</p>
                      </div>)}
                  </>)}
                {post.type === "FOOD_RESCUE" && (<div className="space-y-4">
                    <div className="flex items-start">
                      <h3 className="font-semibold text-lg">Food Rescue Opportunity</h3>
                    </div>
                    <p>{post.content}</p>
                    {post.location && typeof post.location === 'string' && (<p className="text-sm text-muted-foreground flex items-center gap-2">
                        <MapPin className="h-4 w-4"/>
                        {post.location}
                      </p>)}
                  </div>)}
                {post.type === "COOKING_TIP" && (<div className="space-y-4">
                    <div className="flex items-start">
        ...
        {(filteredPosts === null || filteredPosts === void 0 ? void 0 : filteredPosts.length) === 0 && (<div className="text-center py-12">
