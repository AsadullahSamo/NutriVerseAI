...
export function AddEquipmentDialog() {
    ...
    const onSubmit = async (data) => {
        ...
        setLoading(true);
        try {
            const formattedData = Object.assign(Object.assign({}, data), { userId: user.id, lastMaintenanceDate: data.purchaseDate ? new Date().toISOString() : null, purchaseDate: data.purchaseDate ? new Date(data.purchaseDate).toISOString() : null, maintenanceInterval: data.maintenanceInterval || null });
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="name" {...register('name', { required: 'Name is required' })} placeholder="e.g., Stand Mixer"/>
          ...
          <div className="space-y-2">
            ...
            <Select value={watch('condition')} onValueChange={(value) => setValue('condition', value, { shouldValidate: true })}>
              <SelectTrigger>
                <SelectValue placeholder="Select condition"/>
              </SelectTrigger>
              <SelectContent>
                {conditions.map((condition) => (<SelectItem key={condition} value={condition}>
          ...
          <div className="space-y-2">
            ...
            <Input id="maintenanceInterval" type="number" {...register('maintenanceInterval', {
        setValueAs: (v) => v === "" ? null : parseInt(v),
        min: { value: 1, message: 'Must be at least 1 day' },
    })} placeholder="e.g., 90"/>
            {errors.maintenanceInterval && (<p className="text-sm text-red-500">
