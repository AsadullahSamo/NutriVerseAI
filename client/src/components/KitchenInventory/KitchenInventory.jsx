...
export function KitchenInventory() {
    ...
    const handleEquipmentCardClick = (item) => {
        setSelectedEquipment(item);
    ...
    return (<div className="container mx-auto p-4">
      ...
      <Tabs defaultValue="inventory" className="w-full">
        ...
        <TabsContent value="inventory">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {state.equipment.map((item) => (<motion.div key={item.id} whileHover={{ scale: 1.02 }} className="cursor-pointer">
                <Card>
                  ...
                  <CardContent onClick={() => handleEquipmentCardClick(item)}>
                    ...
                    {item.lastMaintenanceDate && (<p className="text-sm text-gray-600">
        ...
        <TabsContent value="analysis">
          <Card className="mb-4">
            <CardContent className="pt-4">
              <Button onClick={handleAnalyze} disabled={state.loading} className="w-full md:w-auto">
                {state.loading ? (<>
          ...
          {state.analysis && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  <h3 className="text-lg font-semibold">Maintenance Recommendations</h3>
                </CardHeader>
                <CardContent>
                  {state.analysis.maintenanceRecommendations.map((rec, idx) => (<Alert key={idx} className="mb-2">
              ...
              <Card>
                ...
                <CardContent>
                  {state.analysis.shoppingRecommendations.map((rec, idx) => (<Alert key={idx} className="mb-2">
                      <AlertTitle>{rec.itemName}</AlertTitle>
                      <AlertDescription>
                        {rec.reason}
                        {rec.estimatedPrice && (<span className="block text-sm text-gray-600">
              ...
              <Card className="md:col-span-2">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {state.analysis.recipeRecommendations.map((rec, idx) => (<Card key={idx}>
                        <CardContent className="pt-4">
                          ...
                          <div className="mt-2 text-sm text-gray-600">
                            ...
                            <ul className="list-disc list-inside">
                              {rec.requiredEquipment.map((eq, i) => (<li key={i}>{eq}</li>))}
                            </ul>
                          </div>
                        </CardContent>
                      </Card>))}
          ...
          {selectedEquipment ? (<Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {maintenanceTips.map((tip, idx) => (<Alert key={idx}>
      ...
      {equipmentToDelete && (<DeleteEquipmentDialog equipment={equipmentToDelete} open={!!equipmentToDelete} onOpenChange={(open) => !open && setEquipmentToDelete(null)}/>)}
    </div>);
}
