...
export function NutritionSummary() {
    ...
    if (!(currentGoal === null || currentGoal === void 0 ? void 0 : currentGoal.progress))
        return null;
    ...
    const filteredProgressData = dateRange.map(date => {
        ...
        const dayConsumption = (consumptionHistory || [])
            ...
            .reduce((acc, curr) => {
            var _a;
            const recipe = ((_a = curr.recipe) === null || _a === void 0 ? void 0 : _a.nutritionInfo) || { calories: 0, protein: 0, carbs: 0, fat: 0 };
            return {
    ...
    return (<Card>
      ...
      <CardContent className="space-y-6">
        ...
        {consumptionHistory && consumptionHistory.length > 0 ? (<Card className="p-4">
            <h4 className="text-sm font-medium mb-3">Meal Distribution</h4>
            <div className="flex flex-wrap gap-2">
              {Object.entries(mealDistribution).map(([type, count]) => (<Button key={type} variant={mealType === type ? "default" : "outline"} size="sm" className="flex items-center gap-2" onClick={() => setMealType(type)}>
                  <span className="capitalize">{type}</span>
                  <Badge variant="secondary">{count}</Badge>
                </Button>))}
            </div>
          </Card>) : (<Card className="p-6 flex flex-col items-center justify-center text-center space-y-2">
        ...
        <div className="space-y-4">
          <div>
            ...
            <LineChart data={filteredProgressData} categories={["calories", "targetCalories"]} index="date" colors={["#f97316", "#94a3b8"]} valueFormatter={(value) => `${value} kcal`}/>
          </div>

          <div className="grid grid-cols-3 gap-4">
            <div>
              <h4 className="text-sm font-medium mb-3">Protein</h4>
              <LineChart data={filteredProgressData} categories={["protein", "targetProtein"]} index="date" colors={["#22c55e", "#94a3b8"]} valueFormatter={(value) => `${value}g`}/>
            </div>
            <div>
              <h4 className="text-sm font-medium mb-3">Carbs</h4>
              <LineChart data={filteredProgressData} categories={["carbs", "targetCarbs"]} index="date" colors={["#3b82f6", "#94a3b8"]} valueFormatter={(value) => `${value}g`}/>
            </div>
            <div>
              <h4 className="text-sm font-medium mb-3">Fat</h4>
              <LineChart data={filteredProgressData} categories={["fat", "targetFat"]} index="date" colors={["#8b5cf6", "#94a3b8"]} valueFormatter={(value) => `${value}g`}/>
            </div>
          </div>
        </div>
      </CardContent>
