...
const ScrollBar = React.forwardRef((_a, ref) => {
    ...
    return (<ScrollAreaPrimitive.ScrollAreaScrollbar ref={ref} orientation={orientation} className={cn("flex touch-none select-none transition-colors", orientation === "vertical" &&
            "h-full w-2.5 border-l border-l-transparent p-[1px] ml-2", orientation === "horizontal" &&
            "h-2.5 flex-col border-t border-t-transparent p-[1px] mb-2", className)} {...props}>
    <ScrollAreaPrimitive.ScrollAreaThumb className={cn("relative flex-1 rounded-full bg-border hover:bg-border/80 transition-colors", orientation === "vertical" && "w-1.5", orientation === "horizontal" && "h-1.5")}/>
