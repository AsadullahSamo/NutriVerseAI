...
export function KitchenStorage() {
  ...
  const fetchSpoilagePredictions = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to get spoilage predictions")
  ...
  const fetchLayoutOptimization = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to optimize layout")
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            {state.smartShoppingList.length > 0 && (
              <ul className="mt-4 space-y-2">
                {state.smartShoppingList.map((item, index) => (
        ...
        <Card>
          ...
          <CardContent>
            <Button
              onClick={fetchSpoilagePredictions}
              disabled={loading}
              className="w-full"
            >
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            ...
            {state.spoilagePredictions.length > 0 && (
              <ul className="mt-4 space-y-2">
                {state.spoilagePredictions.map(prediction => {
                  const item = state.items.find(i => i.id === prediction.itemId)
                  return (
                    <li key={prediction.itemId} className="space-y-1">
                      <div className="flex justify-between">
                        <span>{item?.name}</span>
                        <span
                          className={
                            prediction.daysUntilSpoilage < 7
                              ? "text-red-500"
                              : "text-green-500"
                          }
                        >
        ...
        <Card>
          ...
          <CardContent>
            <Button
              onClick={fetchLayoutOptimization}
              disabled={loading}
              className="w-full"
            >
              {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            ...
            {state.layoutOptimization.moves.length > 0 && (
              <ul className="mt-4 space-y-2">
                {state.layoutOptimization.moves.map(move => {
                  const item = state.items.find(i => i.id === move.itemId)
                  const location = state.locations.find(
                    l => l.id === move.newLocationId
                  )
                  return (
                    <li key={move.itemId} className="space-y-1">
                      <div className="flex justify-between">
                        <span>{item?.name}</span>
                        <span>→</span>
            ...
            {state.locations.map(location => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {location.type} - {location.currentItems}/
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {location?.name} - Quantity: {item.quantity}
                    </p>
                  </div>
                  <Button
                    variant="outline"
