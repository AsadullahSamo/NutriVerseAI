...
import { CreateCulturalRecipeDialogProps } from "@/types";
import { generateCulturalRecipeDetails } from "@/lib/generateCulturalRecipeDetails";

export function CreateCulturalRecipeDialog({ open, onOpenChange, onSuccess, cuisine }: CreateCulturalRecipeDialogProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const response = await fetch("/api/cultural-recipes", {
        ...
        body: JSON.stringify({
          ...data,
          authenticIngredients: data.authenticIngredients.split('\n').map(i => i.trim()).filter(Boolean),
