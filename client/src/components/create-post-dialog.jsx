...
export function CreatePostDialog({ trigger }) {
        ...
        mutationFn: async (data) => {
            try {
                const payload = {
                    ...
                    type: data.type,
                    userId: data.userId,
                    username: (user === null || user === void 0 ? void 0 : user.username) || (user === null || user === void 0 ? void 0 : user.name) || 'Anonymous'
                };
    ...
    const handleCreatePost = () => {
        ...
        const postData = Object.assign({ content,
            type, userId: user === null || user === void 0 ? void 0 : user.id }, (type === "RECIPE_SHARE" && selectedRecipeId ? { recipeId: parseInt(selectedRecipeId) } : {}));
      ...
      <DialogContent>
        ...
        <div className="space-y-4 mt-4">
          ...
          {type === "RECIPE_SHARE" && (<Select value={selectedRecipeId} onValueChange={setSelectedRecipeId}>
              ...
              <SelectContent>
                {recipes && recipes.map((recipe) => (<SelectItem key={recipe.id} value={recipe.id.toString()}>
                    {recipe.title}
                  </SelectItem>))}
              </SelectContent>
            </Select>)}

          <Textarea placeholder="What would you like to share?" value={content} onChange={(e) => setContent(e.target.value)} className="min-h-[150px]"/>
          <Button className="w-full" onClick={handleCreatePost} disabled={!content.trim() || createPostMutation.isPending || (type === "RECIPE_SHARE" && !selectedRecipeId)}>
