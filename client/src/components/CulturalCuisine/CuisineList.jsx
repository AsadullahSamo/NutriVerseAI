...
export function CuisineList({ cuisines, onSelectCuisine }) {
    ...
    const generateAIDetails = async (form) => {
        ...
        try {
            ...
            const keyIngredientsInput = form.querySelector('textarea[name="keyIngredients"]');
            const cookingTechniquesInput = form.querySelector('textarea[name="cookingTechniques"]');
            const bannerUrlInput = form.querySelector('input[name="bannerUrl"]');
            // Update name and region if they were empty
            if (!name && nameInput)
                nameInput.value = details.culturalContext.history.split(' ')[0] + ' Cuisine';
            if (!region && regionInput)
                regionInput.value = details.culturalContext.influences.split(' ')[0] + ' Region';
            if (descriptionInput)
            ...
            if (bannerUrlInput)
                bannerUrlInput.value = `https://source.unsplash.com/800x600/?${encodeURIComponent((name || region).toLowerCase() + ' food')}`;
            toast({
    ...
    const handleAddCuisine = async (event) => {
        ...
        try {
            ...
            const newCuisine = {
                ...
                keyIngredients: formData.get('keyIngredients').split('\n').map(i => i.trim()).filter(Boolean),
                cookingTechniques: formData.get('cookingTechniques').split('\n').map(t => t.trim()).filter(Boolean),
                culturalContext: {
                    history: "To be added",
    ...
    return (<div>
          ...
          <ScrollArea className="max-h-[80vh] px-6">
            ...
            if (form)
                ...
                <div className="space-y-2.5">
                  ...
                  <Input name="bannerUrl" placeholder="https://example.com/banner.jpg" type="url" required/>
                ...
                <div className="space-y-2.5">
                  ...
                  <Textarea name="keyIngredients" placeholder="Enter each ingredient on a new line&#10;e.g.,&#10;Rice&#10;Garlic&#10;Ginger&#10;Soy Sauce" required className="min-h-[100px]"/>
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2.5">
                  ...
                  <Textarea name="cookingTechniques" placeholder="Enter each technique on a new line&#10;e.g.,&#10;Stir-frying&#10;Steaming&#10;Grilling" required className="min-h-[100px]"/>
                  <p className="text-xs text-muted-foreground">
      ...
      <ScrollArea className="h-[calc(100vh-12rem)]">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
          {visibleCuisines.map((cuisine) => (<Card key={cuisine.id} className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => onSelectCuisine(cuisine.id)}>
              <CardHeader>
