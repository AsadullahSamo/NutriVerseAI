...
import { ArrowLeft, ChevronRight, AlertTriangle, Sparkles, ChefHat, Loader2, Edit, History, Star, UtensilsCrossed, Map, Scroll, ArrowRight, Info, Globe2, Wine, Palette, ListOrdered, Ban, MapPin } from "lucide-react";
import { getTechniqueTips, getSubstitutions, getPairings, getEtiquette, getCulturalContext } from "@ai-services/cultural-cuisine-service";
...
export function RecipeDetails({ recipe, cuisine, onBack }) {
    ...
    const canDeleteRecipe = (user === null || user === void 0 ? void 0 : user.id) === recipe.createdBy;
    // Add console logging at the start of component
    console.log('Full recipe details:', Object.assign(Object.assign({}, recipe), { hasImageUrl: !!recipe.image_url, fallbackImageUrl: `https://source.unsplash.com/1200x800/?${encodeURIComponent(recipe.name.toLowerCase() + ' italian food')}` }));
    ...
    const fetchTechniqueTips = async () => {
        if (!recipe || !cuisine || techniqueTips.length > 0)
            return;
        setIsAnalyzing(true);
        try {
    ...
    const handleUpdateRecipe = async (updatedData) => {
        try {
            ...
            const formattedData = Object.assign(Object.assign({}, updatedData), { instructions: Array.isArray(updatedData.instructions)
                    ? Object.fromEntries(updatedData.instructions.map((inst, i) => [i.toString(), inst]))
                    : updatedData.instructions, authenticIngredients: Array.isArray(updatedData.authenticIngredients)
    ...
    const renderModernAdaptations = () => {
        var _a;
        if (!((_a = authenticityAnalysis === null || authenticityAnalysis === void 0 ? void 0 : authenticityAnalysis.modernAdaptations) === null || _a === void 0 ? void 0 : _a.length)) {
            return null;
        }
        return (<div className="space-y-2">
        <h4 className="text-sm font-medium">Modern Adaptations</h4>
        <div className="flex flex-wrap gap-1">
          {authenticityAnalysis.modernAdaptations.map((adaptation, i) => (<Badge key={i} variant="secondary">{adaptation}</Badge>))}
        </div>
      </div>);
    };
    const generateAIDetails = async () => {
        ...
        try {
            ...
            const ingredients = details.ingredients.map(ing => `${ing.amount} ${ing.item}${ing.notes ? ` (${ing.notes})` : ''}`).join(', ');
            // Update form fields
          ...
          <div className="grid gap-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={recipe.difficulty === 'beginner' ? 'default' :
            recipe.difficulty === 'intermediate' ? 'secondary' : 'destructive'}>
                ...
                <div className="relative aspect-[16/6] w-full overflow-hidden rounded-lg">
                  {recipeDetails.image_url ? (<img src={recipeDetails.image_url} alt={recipeDetails.name} className="object-cover w-full h-full" onError={(e) => {
                e.currentTarget.src = getFallbackImageUrl(recipeDetails.name);
            }}/>) : (<img src={getFallbackImageUrl(recipeDetails.name)} alt={recipeDetails.name} className="object-cover w-full h-full"/>)}
              ...
              <Tabs defaultValue="instructions" className="w-full">
                ...
                <TabsContent value="instructions">
                  ...
                  <ol className="space-y-2">
                    {Array.isArray(recipe.instructions) ? (recipe.instructions.map((step, i) => (<li key={i} className="flex gap-4 items-start">
                          ...
                          <div>{step}</div>
                        </li>))) : recipe.instructions && typeof recipe.instructions === 'object' ? (Object.values(recipe.instructions).map((step, i) => (<li key={i} className="flex gap-4 items-start">
                          <div className="bg-primary text-primary-foreground rounded-full w-6 h-6 flex items-center justify-center flex-shrink-0 mt-0.5">
                            {i + 1}
                          </div>
                          <div>{step}</div>
                  ...
                  <div className="space-y-4 mt-2">
                    ...
                    <div>
                      <div className="flex justify-between items-center">
                        ...
                        <Button variant="outline" size="sm" onClick={() => setIsEditingIngredients(true)}>
                          <Edit className="h-4 w-4 mr-2"/>
                          {hasIngredients ? 'Edit Ingredients' : 'Add Ingredients'}
                        </Button>
                      </div>
                      <ul className="space-y-3 mt-4">
                        {Array.isArray(recipe.authenticIngredients) ? (recipe.authenticIngredients.map((ingredient, i) => (<li key={i} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/10 transition-colors">
                              <div className="flex items-center gap-2">
                                <ChevronRight className="h-4 w-4 text-primary"/>
                                <span className="font-medium">{ingredient}</span>
                              </div>
                              {substitutions.some(s => s.original === ingredient) && (<Badge variant="outline" className="animate-fadeIn flex items-center gap-1.5">
                                  <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"/>
                                  <span>Has substitution</span>
                                </Badge>)}
                            </li>))) : recipe.authenticIngredients && typeof recipe.authenticIngredients === 'object' ? (Object.entries(recipe.authenticIngredients).map(([name, quantity], i) => (<li key={i} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/10 transition-colors">
                              <div className="flex items-center gap-2">
                                <ChevronRight className="h-4 w-4 text-primary"/>
                                <span className="font-medium">{name}: {quantity}</span>
                              </div>
                              {substitutions.some(s => s.original === name) && (<Badge variant="outline" className="animate-fadeIn flex items-center gap-1.5">
                                  <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"/>
                                  <span>Has substitution</span>
                                </Badge>)}
                            </li>))) : null}
                    ...
                    {recipe.localSubstitutes && Object.keys(recipe.localSubstitutes).length > 0 && (<Card className="mt-6 overflow-hidden">
                ...
                return (<div key={i} className="p-4 hover:bg-accent/5 transition-colors">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    {substitutionDetails && (<Badge variant={substitutionDetails.flavorImpact === 'minimal' ? 'outline' :
                            substitutionDetails.flavorImpact === 'moderate' ? 'secondary' :
                                'destructive'} className="animate-fadeIn">
                                        {substitutionDetails.flavorImpact} impact
                                      </Badge>)}
                                  </div>
                                  {(substitutionDetails === null || substitutionDetails === void 0 ? void 0 : substitutionDetails.notes) && (<div className="pl-4 border-l-2 border-primary/20 mt-2">
                    ...
                    <Card className="overflow-hidden">
                      <CardHeader className="flex flex-row items-center justify-between space-y-0 bg-muted/50 border-b">
                        ...
                        <Button variant="ghost" size="sm" onClick={fetchSubstitutions} disabled={loading}>
                          {loading ? (<>
                      ...
                      <CardContent className="p-6">
                        {substitutions.length > 0 ? (<div className="grid gap-4 md:grid-cols-2">
                            {substitutions.map((sub, index) => (<div key={index} className="group p-4 rounded-lg border bg-card hover:shadow-md transition-all duration-200">
                                <div className="flex flex-wrap gap-4 items-start justify-between">
                                  ...
                                  <Badge variant={sub.flavorImpact === 'minimal' ? 'outline' :
                    sub.flavorImpact === 'moderate' ? 'secondary' :
                        'destructive'} className="transition-all duration-200 group-hover:scale-105">
                                    <div className="flex items-center gap-1.5">
                                      <div className={`w-2 h-2 rounded-full ${sub.flavorImpact === 'minimal' ? 'bg-primary' :
                    sub.flavorImpact === 'moderate' ? 'bg-secondary' :
                        ...
                        {!user && (<div className="mt-6 pt-6 border-t text-center">
                            <p className="text-sm text-muted-foreground">
                    ...
                    <Card className="overflow-hidden">
                      <CardHeader className="flex flex-row items-center justify-between space-y-0 bg-muted/50 border-b">
                        ...
                        <Button variant="ghost" size="sm" onClick={fetchPairings} disabled={loading} className="ml-2">
                          {loading ? (<>
                              <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                              Loading...
                            </>) : (<>
                      ...
                      <CardContent className="p-6">
                        {pairings ? (<div className="grid gap-6 md:grid-cols-2">
                            {pairings.mainDishes.length > 0 && (<div className="space-y-3 p-4 rounded-lg border bg-card">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <ChefHat className="h-4 w-4 text-primary"/>
                                  Main Dishes
                                </h4>
                                <ul className="space-y-2">
                                  {pairings.mainDishes.map((dish, i) => (<li key={i} className="text-sm flex items-start gap-2">
                            ...
                            {pairings.sideDishes.length > 0 && (<div className="space-y-3 p-4 rounded-lg border bg-card">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <UtensilsCrossed className="h-4 w-4 text-emerald-500"/>
                                  Side Dishes
                                </h4>
                                <ul className="space-y-2">
                                  {pairings.sideDishes.map((dish, i) => (<li key={i} className="text-sm flex items-start gap-2">
                                      <ChevronRight className="h-4 w-4 text-muted-foreground mt-0.5"/>
                                      <span>{dish}</span>
                                    </li>))}
                                </ul>
                            ...
                            {pairings.desserts.length > 0 && (<div className="space-y-3 p-4 rounded-lg border bg-card">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <Sparkles className="h-4 w-4 text-amber-500"/>
                                  Desserts
                                </h4>
                                <ul className="space-y-2">
                                  {pairings.desserts.map((dish, i) => (<li key={i} className="text-sm flex items-start gap-2">
                                      <ChevronRight className="h-4 w-4 text-muted-foreground mt-0.5"/>
                                      <span>{dish}</span>
                                    </li>))}
                                </ul>
                            ...
                            {pairings.beverages.length > 0 && (<div className="space-y-3 p-4 rounded-lg border bg-card">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <Wine className="h-4 w-4 text-indigo-500"/>
                                  Beverages
                                </h4>
                                <ul className="space-y-2">
                                  {pairings.beverages.map((beverage, i) => (<li key={i} className="text-sm flex items-start gap-2">
                                      <ChevronRight className="h-4 w-4 text-muted-foreground mt-0.5"/>
                                      <span>{beverage}</span>
                                    </li>))}
                                </ul>
                    ...
                    <Card className="overflow-hidden">
                      <CardHeader className="flex flex-row items-center justify-between space-y-0 bg-muted/50 border-b">
                        ...
                        <Button variant="ghost" size="sm" onClick={fetchEtiquette} disabled={loading} className="ml-2">
                          {loading ? (<>
                              <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                              Loading...
                            </>) : (<>
                      ...
                      <CardContent className="p-6">
                        {etiquette ? (<div className="grid gap-6 md:grid-cols-2">
                            {etiquette.taboos.length > 0 && (<div className="space-y-3 p-4 rounded-lg border">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <AlertTriangle className="h-4 w-4 text-red-600"/>
                                  <strong>Cultural Taboos</strong>
                                </h4>
                                <ul className="space-y-2">
                                  {etiquette.taboos.map((taboo, i) => (<li key={i} className="text-sm flex items-start gap-2 p-3 rounded-md hover:bg-muted/60 transition-colors">
                            ...
                            {etiquette.customs.length > 0 && (<div className="space-y-3 p-4 rounded-lg border">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <Scroll className="h-4 w-4 text-amber-600"/>
                                  <strong>Traditional Customs</strong>
                                </h4>
                                <ul className="space-y-2">
                                  {etiquette.customs.map((custom, i) => (<li key={i} className="text-sm flex items-start gap-2 p-3 rounded-md hover:bg-muted/60 transition-colors">
                            ...
                            {etiquette.presentation.length > 0 && (<div className="space-y-3 p-4 rounded-lg border">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <Palette className="h-4 w-4 text-emerald-600"/>
                                  <strong>Table Setting & Presentation</strong>
                                </h4>
                                <ul className="space-y-2">
                                  {etiquette.presentation.map((tip, i) => (<li key={i} className="text-sm flex items-start gap-2 p-3 rounded-md hover:bg-muted/60 transition-colors">
                            ...
                            {etiquette.servingOrder.length > 0 && (<div className="space-y-3 p-4 rounded-lg border">
                                <h4 className="font-medium text-sm flex items-center gap-2">
                                  <ListOrdered className="h-4 w-4 text-blue-600"/>
                                  <strong>Serving Order</strong>
                                </h4>
                                <ul className="space-y-2">
                                  {etiquette.servingOrder.map((step, i) => (<li key={i} className="text-sm flex items-start gap-2 p-3 rounded-md hover:bg-muted/60 transition-colors">
                        ...
                        {culturalContext && (<Card className="mt-6 overflow-hidden">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 bg-muted/50 border-b">
                              <div>
                                <CardTitle className="text-base font-semibold flex items-center gap-2">
                                  <Globe2 className="h-4 w-4 text-primary"/>
                            ...
                            <CardContent className="p-6">
                              <div className="grid gap-6 md:grid-cols-2">
                                {culturalContext.history && (<div className="space-y-3 p-4 rounded-lg border">
                                    <h4 className="font-medium text-sm flex items-center gap-2">
                                ...
                                {culturalContext.significance && (<div className="space-y-3 p-4 rounded-lg border">
                                    <h4 className="font-medium text-sm flex items-center gap-2">
                                ...
                                {culturalContext.variations && (<div className="space-y-3 p-4 rounded-lg border">
                                    <h4 className="font-medium text-sm flex items-center gap-2">
            ...
            setIsEditingInstructions(false);
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={generateAIDetails} disabled={isGenerating}>
                        {isGenerating ? (<Loader2 className="h-4 w-4 animate-spin text-green-500"/>) : (<Sparkles className="h-4 w-4 text-green-500"/>)}
                        <span className="ml-2">Generate</span>
                      </Button>
                    </div>
                    <Textarea name="instructions" defaultValue={Array.isArray(recipeDetails.instructions) ?
            recipeDetails.instructions.join('\n') :
            Object.values(recipeDetails.instructions || {}).join('\n')} placeholder="Enter each step on a new line" rows={10}/>
                    <p className="text-sm text-muted-foreground">
          ...
          <DialogContent className="max-h-[90vh] flex flex-col">
            ...
            setIsEditingIngredients(false);
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea name="ingredients" defaultValue={Array.isArray(recipeDetails.authenticIngredients) ?
            recipeDetails.authenticIngredients.join(', ') :
            Object.keys(recipeDetails.authenticIngredients || {}).join(', ')} placeholder="Enter ingredients separated by commas" rows={6}/>
                    <p className="text-sm text-muted-foreground">
