...
export function KitchenEquipment() {
    ...
    const updateAnalytics = async () => {
        ...
        try {
            ...
            const [recsByEquip, maintSchedule, equipRecommendations] = await Promise.all([
                getRecipesByEquipment(convertedEquipment, userPreferences),
                generateMaintenanceSchedule(convertedEquipment, new Date().toISOString(), new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()),
    ...
    return (<div className="container mx-auto py-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="inventory">
          <Card>
            ...
            <CardContent>
              <EquipmentList equipment={equipment.map(convertToKitchenEquipment)} onEquipmentUpdated={fetchEquipment}/>
            </CardContent>
          </Card>
        </TabsContent>

        <TabsContent value="maintenance">
          <MaintenanceScheduleView schedule={maintenanceSchedule} equipment={equipment.map(convertToKitchenEquipment)} onScheduleUpdated={() => updateAnalytics()}/>
        </TabsContent>

        <TabsContent value="shopping">
          <RecommendationsView recommendations={recommendations} onRecommendationAction={(item) => {
        ...
        <TabsContent value="recipes">
          <Card>
            ...
            <CardContent>
              {recipeMatches && (<div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Recipes You Can Make</h3>
                    <ScrollArea className="h-[200px]">
                      <ul className="space-y-2">
                        {recipeMatches.possibleRecipes.map((recipe) => (<li key={recipe.id} className="flex items-center justify-between">
                            <span>{recipe.title}</span>
                            <div className="flex gap-2">
                              {recipe.requiredEquipment.map((eq) => (<Badge key={eq} variant="secondary">{eq}</Badge>))}
                            </div>
                          </li>))}
                      </ul>
                    </ScrollArea>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Recommended Purchases</h3>
                    <ScrollArea className="h-[200px]">
                      <ul className="space-y-2">
                        {recipeMatches.recommendedPurchases.map((rec, i) => (<li key={i} className="border-b pb-2">
      ...
      <AddEquipmentDialog open={showAddDialog} onOpenChange={setShowAddDialog} onEquipmentAdded={() => {
            fetchEquipment();
            updateAnalytics();
        }}/>
    </div>);
