      ...
      <Card className="p-6">
        <div className="space-y-4">
          <div className="flex gap-2">
            <Input value={currentIngredient} onChange={(e) => setCurrentIngredient(e.target.value)} placeholder="Enter an ingredient" onKeyDown={(e) => e.key === "Enter" && addIngredient()} disabled={isLoading} className="flex-1"/>
          ...
          <div className="flex flex-wrap gap-2">
            {ingredients.map((ingredient, index) => (<Badge key={index} variant="secondary" className="cursor-pointer hover:bg-secondary/80 transition-colors px-3 py-1" onClick={() => removeIngredient(index)}>
                {ingredient}
                <X className="h-3 w-3 ml-1"/>
              </Badge>))}
          </div>

          <Button onClick={getRecommendations} disabled={isLoading || ingredients.length === 0} className="w-full relative bg-primary hover:bg-primary/90">
            {isLoading ? (<>
            ...
            <CardContent className="p-6 space-y-4">
              <div>
                ...
                <ul className="list-disc pl-4 space-y-1">
                  {(_b = (_a = recipe.ingredients) === null || _a === void 0 ? void 0 : _a.map((ing, i) => (<li key={i} className="text-muted-foreground">{ing}</li>))) !== null && _b !== void 0 ? _b : <li>No ingredients listed</li>}
                </ul>
              </div>
              ...
              <div>
                ...
                <ol className="list-decimal pl-4 space-y-2">
                  {(_d = (_c = recipe.instructions) === null || _c === void 0 ? void 0 : _c.map((step, i) => (<li key={i} className="text-muted-foreground leading-relaxed">{step}</li>))) !== null && _d !== void 0 ? _d : <li>No instructions provided</li>}
                </ol>
              </div>
