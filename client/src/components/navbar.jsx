      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b bg-background">
        <div className="container flex h-14 items-center">
          ...
          <nav className="hidden md:flex justify-center items-center space-x-6 mx-auto">
            {/* Main Links */}
            {mainLinks.map((link) => (<Link key={link.href} href={link.href} className="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors hover:bg-accent hover:text-accent-foreground">
                <link.icon className="h-4 w-4"/>
                <span>{link.label}</span>
              </Link>))}

            {/* Dropdown Menus */}
            {Object.entries(menuGroups).map(([key, group]) => (<DropdownMenu key={key}>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuGroup>
                    {group.items.map((item) => (<DropdownMenuItem key={item.href} asChild>
          ...
          <div className="flex items-center gap-4 ml-auto pr-4">
            ...
            <Button variant="ghost" size="sm" className="md:hidden inline-flex h-10 w-10 items-center justify-center rounded-md p-0" onClick={() => setIsOpen(!isOpen)}>
              <span className="sr-only">Open menu</span>
              {isOpen ? (<X className="h-5 w-5"/>) : (<Menu className="h-5 w-5"/>)}
            </Button>

            <ThemeToggle />
            {user ? (<UserButton />) : (<div className="hidden md:flex items-center gap-3">
        ...
        {isOpen && (<>
            <div className="fixed inset-0 top-[3.5rem] z-40 bg-background/95 backdrop-blur-sm" onClick={() => setIsOpen(false)}/>
            <div className="fixed inset-x-0 top-[3.5rem] z-50 border-t bg-background shadow-lg">
              <nav className="container flex flex-col divide-y divide-border">
                {/* Main Links */}
                {mainLinks.map((link) => (<Link key={link.href} href={link.href} className="h-12 w-[100%] flex items-center gap-3 px-6 py-4 text-base font-medium transition-colors hover:bg-accent hover:text-accent-foreground" onClick={() => setIsOpen(false)}>
                    <link.icon className="h-5 w-5"/>
                    <span>{link.label}</span>
                  </Link>))}

                {/* Group Links */}
                {Object.values(menuGroups).flatMap(group => group.items.map(item => (<Link key={item.href} href={item.href} className="h-12 w-[100%] flex items-center gap-3 px-6 py-4 text-base font-medium transition-colors hover:bg-accent hover:text-accent-foreground" onClick={() => setIsOpen(false)}>
