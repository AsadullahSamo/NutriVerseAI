...
export function PantryItemCard({ item }) {
    ...
    const carbonFootprint = ((_a = item.sustainabilityInfo) === null || _a === void 0 ? void 0 : _a.carbonFootprint) || 'low';
    const sustainabilityScore = ((_b = item.sustainabilityInfo) === null || _b === void 0 ? void 0 : _b.score) || 0;
    return (<Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-lg font-medium">{item.name}</CardTitle>
        <div className="flex space-x-2">
          <EditPantryItemDialog item={item}/>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => deleteMutation.mutate()}>
      ...
      <CardContent>
        <div className="grid gap-2">
          ...
          {item.category && (<div className="text-sm">
              <span className="text-muted-foreground">Category:</span> {item.category}
            </div>)}
          {item.expiryDate && (<div className="text-sm">
          ...
          <div className="mt-2 pt-2 border-t">
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className={`${sustainabilityScore >= 70 ? 'text-green-600' :
          ...
          {/* Nutrition Info */}
          {item.nutritionInfo && (<div className="mt-2 pt-2 border-t">
