...
export function UserAvatar({ className = "", size = "md" }) {
    const { user } = useAuth();
    const { preferences } = useUserPreferences();
    const [forceRender, setForceRender] = useState(0);
    const [userData, setUserData] = useState(() => ({
        name: (user === null || user === void 0 ? void 0 : user.name) || (user === null || user === void 0 ? void 0 : user.username),
    ...
    useEffect(() => {
        const handleProfileUpdate = () => {
            ...
            if (savedProfile) {
                ...
                setUserData(prev => (Object.assign(Object.assign({}, prev), { name: profile.name || (user === null || user === void 0 ? void 0 : user.name) || (user === null || user === void 0 ? void 0 : user.username), profilePicture: profile.profilePicture })));
            }
        };
    ...
    const initials = useMemo(() => getInitials(userData.name || (user === null || user === void 0 ? void 0 : user.username) || ""), [userData.name, user === null || user === void 0 ? void 0 : user.username]);
    const dimensions = useMemo(() => size === "sm" ? "h-8 w-8" : "h-10 w-10", [size]);
    ...
    return (<div className={cn("relative inline-block", className)}>
      <div className={cn("overflow-hidden rounded-full", dimensions)}>
        {userData.profilePicture ? (<img src={userData.profilePicture} alt={userData.name || user.username} className="h-full w-full object-cover" style={{ aspectRatio: "1/1" }}/>) : (<div className="flex h-full w-full items-center justify-center font-semibold uppercase text-white" style={{ backgroundColor: accentColor }} key={`avatar-${accentColor}-${forceRender}`} // Force re-render when color changes
        >
            {initials}
          </div>)}
      </div>
