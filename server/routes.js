...
export async function registerRoutes(app) {
    ...
    app.get("/api/recipes/consumption-history", isAuthenticated, asyncHandler(async (req, res) => {
        ...
        const history = await storage.getRecipeConsumptionWithDetails(req.user.id, startDate, endDate, mealType);
        res.json(history);
    }));
    ...
    app.post("/api/grocery-lists", isAuthenticated, asyncHandler(async (req, res) => {
        ...
        const list = await storage.createGroceryList(Object.assign(Object.assign({}, validated), { userId: req.user.id, completed: (_a = validated.completed) !== null && _a !== void 0 ? _a : false, expiryDates: (_b = validated.expiryDates) !== null && _b !== void 0 ? _b : null, smartSubstitutions: (_c = validated.smartSubstitutions) !== null && _c !== void 0 ? _c : null }));
    ...
    app.post("/api/pantry", isAuthenticated, asyncHandler(async (req, res) => {
        ...
        const item = await storage.createPantryItem(Object.assign(Object.assign({}, validated), { userId: req.user.id, expiryDate: (_a = validated.expiryDate) !== null && _a !== void 0 ? _a : null, category: (_b = validated.category) !== null && _b !== void 0 ? _b : null }));
    ...
    app.get("/api/nutrition-goals/progress/today", isAuthenticated, asyncHandler(async (req, res) => {
        ...
        const today = new Date().toISOString().split('T')[0];
        const todayProgress = ((_a = currentGoal.progress) === null || _a === void 0 ? void 0 : _a.find((p) => p.date === today)) || {
            date: today,
            calories: 0,
            protein: 0,
            carbs: 0,
    ...
    app.get("/api/nutrition-goals/insights", isAuthenticated, asyncHandler(async (req, res) => {
        ...
        const user = await storage.getUser(req.user.id);
        const preferences = ((_a = user === null || user === void 0 ? void 0 : user.preferences) === null || _a === void 0 ? void 0 : _a.dietaryPreferences) || [];
        ...
        }, currentGoal.progress || [], preferences);
    ...
    app.post('/api/kitchen-equipment', async (req, res) => {
        try {
            const data = Object.assign(Object.assign({}, req.body), { userId: req.user.id, createdAt: new Date(), updatedAt: new Date() });
    ...
    app.delete('/api/kitchen-equipment/:id', async (req, res) => {
        try {
            await db.delete(kitchenEquipment)
                .where(and(eq(kitchenEquipment.id, parseInt(req.params.id)), eq(kitchenEquipment.userId, req.user.id)));
    ...
    app.patch('/api/cultural-recipes/:id', isAuthenticated, async (req, res) => {
        try {
            const _a = req.body, { imageUrl } = _a, otherUpdates = __rest(_a, ["imageUrl"]);
            const [updatedRecipe] = await db.update(culturalRecipes)
                .set(Object.assign(Object.assign({}, otherUpdates), { imageUrl: imageUrl || null, updatedAt: new Date() }))
                .where(eq(culturalRecipes.id, parseInt(req.params.id)))
                .returning();
            if (!updatedRecipe) {
                return res.status(404).json({ error: 'Recipe not found' });
